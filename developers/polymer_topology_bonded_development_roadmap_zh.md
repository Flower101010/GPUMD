...
项目不应在第一阶段直接定位为“重新实现一个完整的 GROMACS 或 LAMMPS”。更合理的定位是：

> 开发一个通用、可扩展、GPU 原生的固定拓扑框架，支持高分子常用的 bonded interactions、拓扑相关的非键排除及特殊相互作用，并能够与 GPUMD 现有势函数或后续机器学习势协同工作。
...
## 3. 为什么不只是 Bond、Angle 和 Dihedral
